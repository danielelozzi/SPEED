...
## ✨ Features
  ...
  * **Un-Enriched Data Support:** New option in the GUI to indicate "un-enriched" data analysis. When this option is selected, the `gaze.csv` (enriched) and `fixations.csv` files become optional, and the analysis adapts to calculate only metrics and generate plots possible with the available data (e.g., pupillometry, blinks, saccades, basic gaze paths).
  ...
  * **Comprehensive Visualizations (PDF):**
      ...
      * Movement path plots.
      * *Note: Plots related to fixations, heatmaps, and movements are generated only if enriched data is used or if the necessary columns are present in the un-enriched data.*
...
| `gaze.csv`         | Enriched gaze data                          | CSV    | Yes                      | No                          |
...
3.  **Select Analysis Mode:**
      * **"Analyze un-enriched data only"**: Select this checkbox if you want to run the analysis only with un-enriched data. In this case, the fields for `gaze.csv` and `fixations.csv` will become disabled and their selection will not be required.
...
├── SPEED\_0\_3\_gui.py           \# The Graphical User Interface application (Updated for v0.3)

├── speed\_script\_10\_events.py  \# The core eye-tracking data analysis logic (Updated for v0.3)
