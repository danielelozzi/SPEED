...
# --- NUOVO: Fragmentation Plot Constants ---
...
def _prepare_data(data_dir: Path, un_enriched_mode: bool, options: dict):
    ...
    except FileNotFoundError as e: raise FileNotFoundError(f"Essential file not found: {e}. Cannot generate video.")

    merged_data = pd.merge_asof(world_timestamps, gaze_df[['timestamp [ns]', 'gaze x [px]', 'gaze y [px]']], on='timestamp [ns]', direction='nearest', tolerance=pd.Timedelta('50ms').value)

    # --- NUOVO: Calcolo Frammentazione per il video ---
    ...
    if options.get('overlay_pupil_plot'):
        try:
            ...
            cols_to_merge = []
            if 'pupil diameter left [mm]' in pupil_df.columns: cols_to_merge.append('pupil diameter left [mm]')
            if 'pupil diameter right [mm]' in pupil_df.columns: cols_to_merge.append('pupil diameter right [mm]')
            if cols_to_merge:
                pupil_df['pupil_diameter_mean'] = pupil_df[cols_to_merge].mean(axis=1)
                cols_to_merge.append('pupil_diameter_mean')
                merged_data = pd.merge_asof(merged_data, pupil_df[['timestamp [ns]'] + cols_to_merge], on='timestamp [ns]', direction='backward')
        except FileNotFoundError: print("WARNING: '3d_eye_states.csv' not found. Pupil plot disabled.")
    ...
    except FileNotFoundError: print("WARNING: 'blinks.csv' not found. Blink overlay disabled.")
    ...
    if options.get('crop_and_correct_perspective'):
        try:
            ...
            corner_cols = ['tl x [px]', 'tl y [px]', 'tr x [px]', 'tr y [px]', 'br x [px]', 'br y [px]', 'bl x [px]', 'bl y [px]']
            merged_data = pd.merge_asof(merged_data, surface_df[['timestamp [ns]'] + corner_cols], on='timestamp [ns]', direction='backward')
        except FileNotFoundError: print("WARNING: Perspective option is active, but 'surface_positions.csv' not found. Option disabled."); options['crop_and_correct_perspective'] = False
...
def _draw_generic_plot(frame: np.ndarray, data_points: list, min_val: float, max_val: float, width: int, height: int, position: tuple, title: str, color: tuple, bg_color: tuple):
    ...
    if not data_points or max_val == min_val: return frame
    ...
    if len(data_points) < 2: return frame
    points = [
        (
...
def _draw_pupil_plot(frame: np.ndarray, plot_data_dict: dict, min_val: float, max_val: float, width: int, height: int, position: tuple):
    ...
    if not plot_data_dict or max_val == min_val: return frame
    x_pos, y_pos = position
    plot_area = frame[y_pos:y_pos+height, x_pos:x_pos+width]
    ...
    for name, data_points in plot_data_dict.items():
        ...
        points = [
            (
                ...
                y_pos + height - int(((val - min_val) / (max_val - min_val)) * (height - 50)) - 10
            ) for i, val in enumerate(data_points) if pd.notna(val)
        ]
        if len(points) > 1: cv2.polylines(frame, [np.array(points, dtype=np.int32)], isClosed=False, color=PUPIL_COLORS[name], thickness=2)
...
def create_custom_video(data_dir: Path, output_dir: Path, subj_name: str, options: dict, un_enriched_mode: bool):
    ...
    if not cap_ext.isOpened(): print(f"ERROR: Cannot open external video: {external_vid_path}"); return
    ...
    if options.get('include_internal_cam'):
        ...
        if internal_vid_path.exists():
            ...
            if not cap_int.isOpened(): print("WARNING: Cannot open internal video, PiP disabled."); options['include_internal_cam'] = False
        else: print("WARNING: Internal video not found, PiP disabled."); options['include_internal_cam'] = False

    total_frames = int(cap_ext.get(cv2.CAP_PROP_FRAME_COUNT)); fps = cap_ext.get(cv2.CAP_PROP_FPS)
    original_w, original_h = int(cap_ext.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap_ext.get(cv2.CAP_PROP_FRAME_HEIGHT))
    ...
    pupil_plot_data = {"Left": [], "Right": [], "Mean": []}; pupil_min, pupil_max = 0, 1
    ...
    # --- NUOVO: Setup for fragmentation plot ---
    frag_plot_data = []; frag_min, frag_max = 0, 1
    if options.get('overlay_fragmentation_plot') and 'gaze_speed_px_per_s' in sync_data.columns:
        ...
        if not all_frag_data.empty: frag_min, frag_max = all_frag_data.min(), all_frag_data.quantile(0.99) # Use 99th percentile to avoid extreme outliers
    ...
    try:
        for frame_idx in tqdm(range(min(total_frames, len(sync_data))), desc="Generating Video"):
            ...
            elif frame.shape[1] != out_w or frame.shape[0] != out_h: frame = cv2.resize(frame, (out_w, out_h))

            # --- OVERLAYS ---
            if options.get('include_internal_cam') and cap_int is not None:
                ret_int, frame_int = cap_int.read()
                if ret_int:
                    pip_h = int(out_h * PIP_SCALE); pip_w = int(frame_int.shape[1] * (pip_h / frame_int.shape[0]))
            ...
            if options.get('overlay_gaze') and pd.notna(frame_data.get('gaze x [px]')):
                ...
                if 0 <= px < out_w and 0 <= py < out_h: cv2.circle(frame, (px, py), GAZE_RADIUS, GAZE_COLOR, GAZE_THICKNESS, cv2.LINE_AA)

            if frame_data.get('is_blinking', False): cv2.putText(frame, "BLINK", (out_w - 150, out_h - 20), cv2.FONT_HERSHEY_TRIPLEX, 1.5, BLINK_TEXT_COLOR, 2)
            ...
            # --- NUOVO: Disegna il grafico della frammentazione ---
            if options.get('overlay_fragmentation_plot'):
                ...
                # Posiziona sotto il grafico della pupilla (se esiste) o in alto a destra
    ...
    except Exception as e: print(f"An error occurred during video generation: {e}"); traceback.print_exc()
    finally:
